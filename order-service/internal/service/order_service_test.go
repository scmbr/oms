...
func (m *MockOrderRepo) Create(ctx context.Context, tx *gorm.DB, userID string, items []models.OrderItem) (*models.Order, error) {
	args := m.Called(ctx, tx, userID, items)
	if args.Get(0) == nil {
...
func (m *MockOutboxRepo) Create(ctx context.Context, tx *gorm.DB, event *models.OutboxEvent) error {
...
func (m *MockOutboxRepo) MarkAsSent(ctx context.Context, tx *gorm.DB, eventID string) error {
	return m.Called(ctx, tx, eventID).Error(0)
}
...
func TestGetOrder_Success(t *testing.T) {
	...
	svc := service.NewOrderService(repos, txManager)
